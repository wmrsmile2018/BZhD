    ...
    for(var i = 0; i < current_question.ans.length; i++) {
      ...
      if(!i) {
        ...
        for(var t = 0; t < current_question.image_q.length; t++) {
          ...
          $(img1).addClass("arr_img").data("index", t).attr("src",`data:image/${extension};base64,${fs.readFileSync(path.join(__dirname, current_question.image_q[t].image_data)).toString("base64")}`);
